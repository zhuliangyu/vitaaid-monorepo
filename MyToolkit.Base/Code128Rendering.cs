...
namespace MyToolkit.Base
{
	...
	public static class Code128Rendering
	{
      ...
      public static Image MakeBarcodeImage( string InputData, int BarWeight, bool AddQuietZone, bool ShowText = false )
      {
         ...
         Image myimg = new System.Drawing.Bitmap(width, height + ((ShowText) ? TextHeight : 0));
         using (Graphics gr = Graphics.FromImage(myimg))
         {

            // set to white so we don't have to fill the spaces with white
             gr.FillRectangle(System.Drawing.Brushes.White, 0, 0, width, height + ((ShowText) ? TextHeight : 0));
            ...
            if (ShowText)
                gr.DrawString(InputData, new Font(FontFamily.GenericSansSerif, 12, FontStyle.Regular), new SolidBrush(Color.Black), cQuietWidth, height + 1);
         }
