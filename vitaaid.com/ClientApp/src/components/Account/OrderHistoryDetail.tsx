...
import { getOrderDetail } from 'model/Member';
import {
  AccountPageType,
  accountPageIdx,
  profile_page,
...
export const OrderHistoryDetail = () => {
  ...
  React.useEffect(() => {
    async function fetchData() {
      const data = await getOrderDetail(account.customerCode, orderNo);
      setOrderData(data);
      setLoadingState(0);
    }
  ...
  return (
    <React.Fragment>
      <div className="content-main-body ">
        <BreadCrumbsBlock
          subNodes={['Order History', 'Order Detail']}
          actions={[
            () => {
        ...
        />
        <div className="order-hisory-detail">
          <div className="row">
