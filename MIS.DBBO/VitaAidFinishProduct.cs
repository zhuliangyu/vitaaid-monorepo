...
namespace MIS.DBBO
{
  public class VitaAidFinishProduct : VitaAidProduct
  {
    ...
    public override int OverdueLevelFPArea
    {
      get
      {
        if (ReleasedDate == SysVal.NilDate) return 0;
        var diff = DateTime.Now - ReleasedDate;
        if (diff.TotalDays < 2) return 0;
        else if (diff.TotalDays >= 2 && diff.TotalDays < 3) return 1;
        else return 2;
      }
    }
    public override string ReleasedDays
    {
      get {
        if (ReleasedDate == SysVal.NilDate) return "";
        return Math.Floor((DateTime.Now - ReleasedDate).TotalDays).ToString() + " days";
      }
    }
    ...
    public virtual double dTotalCost
    {
      get => Math.Round(dPackageCost + dRawMaterialCost, 2, MidpointRounding.AwayFromZero);
    }
